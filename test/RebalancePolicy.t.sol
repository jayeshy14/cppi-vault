...
contract RebalancePolicyTest is Test {
    ...
    function test_minInterval_gatesEverything() public view {
        ...
        assertEq(uint8(classify(T0, T0 + 30 minutes, 2000, 0)), uint8(RebalancePolicy.Trigger.None));
    }

    function test_emergency_onLargeDrift_beforeCadence() public view {
        // 2h since last (past minInterval, well before cadence), drift 5%
        assertEq(uint8(classify(T0, T0 + 2 hours, 500, 5000)), uint8(RebalancePolicy.Trigger.Emergency));
    }

    function test_emergency_onLowCushion() public view {
        // drift small but cushion at 2% of nav: crash de-risk path
        assertEq(uint8(classify(T0, T0 + 2 hours, 50, 200)), uint8(RebalancePolicy.Trigger.Emergency));
    }

    function test_scheduled_needsCadenceAndSmallDrift() public view {
        // past cadence with 3% drift
        assertEq(uint8(classify(T0, T0 + 1 days, 300, 5000)), uint8(RebalancePolicy.Trigger.Scheduled));
        // past cadence, drift below band: nothing
        assertEq(uint8(classify(T0, T0 + 1 days, 100, 5000)), uint8(RebalancePolicy.Trigger.None));
        // drift fine, cadence not elapsed: nothing
        assertEq(uint8(classify(T0, T0 + 2 hours, 300, 5000)), uint8(RebalancePolicy.Trigger.None));
    }

    function test_firstRebalance_ignoresTiming() public view {
        assertEq(uint8(classify(0, T0, 300, 5000)), uint8(RebalancePolicy.Trigger.Scheduled));
    }

    /// forge-config: default.fuzz.runs = 1024
    function testFuzz_emergencyDominatesWhenPastMinInterval(uint256 drift, uint256 cushionBps) public view {
        drift = bound(drift, cfg.driftLargeBps, 10_000);
        cushionBps = bound(cushionBps, 0, 10_000);
        assertEq(uint8(classify(T0, T0 + cfg.minInterval, drift, cushionBps)), uint8(RebalancePolicy.Trigger.Emergency));
    }
}
