...
contract CPPIController {
    ...
    function startTerm(uint64 termStart, uint64 termEnd, uint256 nav) external onlyVault {
        ...
        FloorPolicy.initialize(floorState, floorConfig, nav);
        termActive = true;
        lastRebalanceAt = 0;
        unchecked {
            ++termNumber;
        }
        emit TermStarted(termNumber, termStart, termEnd, nav, floorState.protectedAmount);
    }

    /// @notice Settle a matured term. Emits realized shortfall (target: zero).
    function settleTerm(uint256 nav) external onlyVault returns (uint256 shortfall) {
        if (!termActive) revert TermNotActive();
        if (block.timestamp < floorConfig.termEnd) revert TermNotMatured();
        uint256 protectedAmount = floorState.protectedAmount;
    ...
    ///         monotone clamp.
    function assess(uint256 nav, uint256 riskyValue, uint256 rawRateWad)
        external
        onlyVault
        returns (Assessment memory a)
    {
        ...
        a.floor = floorState.currentFloor(floorConfig, nav, rate, block.timestamp);
